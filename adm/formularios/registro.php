...
if (!empty($_POST)) {
	...
	if ($nome && $email && $username && $senha) {
		...
		$codUsuario = $usuarioDao->registro($nome, md5_base64($senha) . $hash, $username, 1, $avatar);
		$contatoDao = new ContatoDao();
		//Cadastra o E-Mail do caboclo, campo tipoContato (E-Mail = 1)
		$contatoDao->insereContato($contato, 1, $codUsuario);
	}
...
function uploadAvatar($campo) {
		...
		return $upload->salvar("../img/", $campo); 
}
